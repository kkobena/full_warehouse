...
@Component({
  ...
  imports: [CommonModule, FormsModule, Button, Select, Textarea]
})
export class CloturerAvoirModalComponent {
  ...
  protected readonly canConfirm = computed(() => !!this.modeCloture());
  ...
  protected readonly modeClotureOptions: { label: string; value: ModeClotureAvoir; icon: string; description: string }[] = [
   ...
   // { label: "Bon d'avoir", value: 'BON_AVOIR', icon: 'pi pi-ticket', description: "Un bon d'avoir est remis au client, utilisable sur une prochaine vente." },
  ...
  //  { label: 'Compensation vente', value: 'COMPENSATION_VENTE', icon: 'pi pi-arrow-right-arrow-left', description: "Le montant est imputé directement sur la prochaine vente du client." },
  ];
  ...
  protected confirm(): void {
    ...
    this.api.cloturerAvoir(this.document.id, request)
