...
""";
  ...
  @Override
  public List<ProductActivityDTO> getProductActivity(
  ...
  private ProductActivityDTO buildByDate(
      LocalDate mvtDate, List<AbstractProduitActivity> activities, Long produitId) {
    ...
    activities.stream()
        ...
        .forEach(
            abstractProduitActivity -> {
              ...
              } else if (abstractProduitActivity
                  instanceof DeconditionActivityDTO deconditionActivity) {
                if (deconditionActivity.getTypeDeconditionnement()
                  ...
                  instanceof AjustementActivityDTO ajustementActivity) {
...
// a revoir
  ...
  private List<VenteActivityDTO> getProduitVente(
    ...
    List<Tuple> query =
        this.em
            ...
            .getResultList();
    return query.stream()
        .map(e -> buildVenteActivity(e, canceled))
        .filter(Objects::nonNull)
        .toList();
  }

  private List<AjustementActivityDTO> getProduitAjustement(
      Long produitId, LocalDate fromDate, LocalDate toDate) {

    return execQuery(AJUSTEMENT_QUERY, produitId, fromDate, toDate).stream()
        .map(this::buildAjustementActivity)
        .filter(Objects::nonNull)
        .toList();
  }

  private List<DeconditionActivityDTO> getDeconditionActivities(
      Long produitId, LocalDate fromDate, LocalDate toDate) {
    return execQuery(DECONDITIONNEMENT_QUERY, produitId, fromDate, toDate).stream()
        .map(this::buildDeconditionActivity)
        .filter(Objects::nonNull)
        .toList();
  }

  private List<OrderActivityDTO> getOrderActivities(
      Long produitId, LocalDate fromDate, LocalDate toDate) {
    return execQuery(ENTRY_QUERY, produitId, fromDate, toDate).stream()
        .map(this::buildOrderActivity)
        .filter(Objects::nonNull)
        .toList();
  }

  private List<InventoryActivityDTO> getInventoriesActivities(
      Long produitId, LocalDate fromDate, LocalDate toDate) {
    return execQuery(INVENTORY_QUERY, produitId, fromDate, toDate).stream()
        .map(this::buildInventoryActivityActivity)
        .filter(Objects::nonNull)
        .toList();
  }

  private List<RetourBonActivityDTO> getRetourBonActivities(
      Long produitId, LocalDate fromDate, LocalDate toDate) {
    return execQuery(RETOUR_BON_QUERY, produitId, fromDate, toDate).stream()
        .map(this::buildRetourBonActivity)
        .filter(Objects::nonNull)
        .toList();
  }

  private List<Tuple> execQuery(
      String sqlQuery, Long produitId, LocalDate fromDate, LocalDate toDate) {
    return this.em
        .createNativeQuery(sqlQuery, Tuple.class)
        .setParameter(1, produitId)
        .setParameter(2, java.sql.Date.valueOf(fromDate))
        .setParameter(3, java.sql.Date.valueOf(toDate))
        .getResultList();
  ...
  private Pair<Integer, Integer> getInitAfterStock(Tuple tuple) {
    return new MutablePair<>(
        tuple.get("init_stock", Integer.class), tuple.get("after_stock", Integer.class));
  }
  ...
  private Pair<Integer, Integer> getInitAfterStock(
      AbstractProduitActivity abstractProduit, Long produitId, LocalDate fromDate, String order) {
    ...
    if (abstractProduit instanceof VenteActivityDTO) {
      ...
      return getInitAfterStock(
          String.format(DECONDITIONNEMENT_STOCK_QUERY, order), produitId, fromDate);
