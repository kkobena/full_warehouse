...
public interface RetourDepotRepository extends JpaRepository<RetourDepot, Integer>, JpaSpecificationExecutor<RetourDepot> {
    ...
    Page<RetourDepot> findAllByDateRange(@Param("depotId") Integer depotId,
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            if (depotId != null) {
                predicates = criteriaBuilder.and(predicates,
                    criteriaBuilder.equal(root.get(RetourDepot_.depot).get(Magasin_.id), depotId));
            }
            if (dtStart != null && dtEnd != null) {
                predicates = criteriaBuilder.and(predicates,
                    criteriaBuilder.between(root.get(RetourDepot_.dateMtv), dtStart, dtEnd));
            }
            return predicates;
        };
    }
