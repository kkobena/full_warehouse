...
public class TableauResource {
  ...
  public ResponseEntity<TableauDTO> create(@Valid @RequestBody TableauDTO tableau)
      throws URISyntaxException {
    log.debug("REST request to save tableaux : {}", tableau);
    if (tableau.getId() != null) {
      throw new BadRequestAlertException(
          "A new tableau cannot already have an ID", ENTITY_NAME, "idexists");
    }
    TableauDTO result = tableauService.save(tableau);
    return ResponseEntity.created(new URI("/api/tableaux/" + result.getId()))
        .headers(
  ...
  @PutMapping("/tableaux")
  public ResponseEntity<TableauDTO> update(@Valid @RequestBody TableauDTO tableauDTO)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, tableauDTO.getId().toString()))
        .body(result);
  }
