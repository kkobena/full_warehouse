...
export default class StockReportsComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.active() === 'stock-alerts' && !this.showStockAlerts()) {
      if (this.showStockValuation()) {
        this.active.set('stock-valuation');
      } else if (this.showRecapProduit()) {
        this.active.set('recap-produit-vendu');
      } else if (this.showStockAbc()) {
        this.active.set('stock-abc');
      } else if (this.showDemarque()) {
        this.active.set('demarque');
      }
    }
  }
}
