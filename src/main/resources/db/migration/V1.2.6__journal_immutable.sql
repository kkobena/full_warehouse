...
INSERT INTO stock_produit_snapshot (id, produit_id, storage_id, snapshot_date, qty_stock,
                                    source_type)
SELECT nextval('stock_produit_snapshot_id_seq'),
       produit_id,
       storage_id,
       NOW(),
       qty_stock,
...
CREATE OR REPLACE FUNCTION fn_stock_at_time(
  ...
  STABLE AS
$$
WITH last_snapshot AS (SELECT qty_stock, snapshot_date
                       ...
                       LIMIT 1),
     deltas AS (SELECT COALESCE(SUM(quantity_after - quantity_befor), 0) AS total_delta
                FROM inventory_transaction
                WHERE produit_id = p_produit_id
                  AND storage_id = p_storage_id
                  AND mouvement_type <> 'INVENTAIRE'
                  AND created_at >
                      (SELECT COALESCE(snapshot_date, '-infinity'::timestamptz) FROM last_snapshot)
                  AND created_at <= p_at)
SELECT GREATEST(
...
FROM repartition_stock_produit r
       ...
       JOIN produit prod ON prod.id = sp_src.produit_id
       LEFT JOIN fournisseur_produit fp
                 ON fp.produit_id = sp_src.produit_id AND
                    prod.fournisseur_produit_principal_id = fp.id
...
FROM repartition_stock_produit r
       ...
       JOIN produit prod ON prod.id = sp_dst.produit_id
       LEFT JOIN fournisseur_produit fp
                 ON fp.produit_id = sp_dst.produit_id AND
                    prod.fournisseur_produit_principal_id = fp.id
...
AS
$$
WITH qty AS (SELECT fn_stock_at_time(p_produit_id, p_storage_id, p_at) AS q),
     last_tx AS (SELECT cost_amount, regular_unit_price
                 FROM inventory_transaction
                 WHERE produit_id = p_produit_id
                   AND storage_id = p_storage_id
                   AND created_at <= p_at
...
AS
$$
WITH scope AS (SELECT sp.produit_id, sp.storage_id
               FROM stock_produit sp
     ...
     last_snapshot AS (SELECT DISTINCT ON (sn.produit_id, sn.storage_id) sn.produit_id,
                                                                         sn.storage_id,
                                                                         sn.qty_stock     AS snap_qty,
                                                                         sn.snapshot_date AS snap_date
                       FROM stock_produit_snapshot sn
                              JOIN scope sc ON sc.produit_id = sn.produit_id AND
                                               sc.storage_id = sn.storage_id
                       WHERE sn.snapshot_date <= p_at
                       ORDER BY sn.produit_id, sn.storage_id, sn.snapshot_date DESC),
     deltas AS (SELECT it.produit_id,
                       it.storage_id,
                       COALESCE(SUM(it.quantity_after - it.quantity_befor), 0) AS total_delta
                FROM inventory_transaction it
                       JOIN scope sc
                            ON sc.produit_id = it.produit_id AND sc.storage_id = it.storage_id
                       LEFT JOIN last_snapshot ls
                                 ON ls.produit_id = it.produit_id AND ls.storage_id = it.storage_id
                WHERE it.mouvement_type <> 'INVENTAIRE'
                  AND it.created_at > COALESCE(ls.snap_date, '-infinity'::timestamptz)
                  AND it.created_at <= p_at
                GROUP BY it.produit_id, it.storage_id),
     qty AS (SELECT sc.produit_id,
                    sc.storage_id,
                    GREATEST(COALESCE(ls.snap_qty, 0) + COALESCE(d.total_delta, 0), 0)::INT AS q
             FROM scope sc
                    LEFT JOIN last_snapshot ls
                              ON ls.produit_id = sc.produit_id AND ls.storage_id = sc.storage_id
                    LEFT JOIN deltas d
                              ON d.produit_id = sc.produit_id AND d.storage_id = sc.storage_id),
     last_price AS (SELECT DISTINCT ON (it.produit_id, it.storage_id) it.produit_id,
                                                                      it.storage_id,
...
WITH scope AS (SELECT sp.produit_id, sp.storage_id
               ...
               WHERE s.magasin_id = p_magasin_id
                 AND (p_produit_ids IS NULL OR sp.produit_id = ANY (p_produit_ids))),
     val_debut AS (SELECT v.produit_id,
                          v.storage_id,
     ...
     val_fin AS (SELECT v.produit_id,
                        v.storage_id,
     ...
     mvt AS (SELECT it.produit_id,
                    ...
                    COALESCE(SUM(CASE
                                   WHEN (it.quantity_after - it.quantity_befor) > 0
                                     AND
                                        it.mouvement_type NOT IN ('AJUSTEMENT_IN', 'AJUSTEMENT_OUT')
                                     THEN (it.quantity_after - it.quantity_befor) END),
                             0) AS entrees,
                    COALESCE(SUM(CASE
                                   WHEN (it.quantity_after - it.quantity_befor) < 0
                                     AND
                                        it.mouvement_type NOT IN ('AJUSTEMENT_IN', 'AJUSTEMENT_OUT')
                                     THEN ABS(it.quantity_after - it.quantity_befor) END),
                             0) AS sorties,
                    COALESCE(SUM(CASE
