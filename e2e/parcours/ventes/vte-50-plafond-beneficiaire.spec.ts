 ...
 * L'organisme rembourse 70 % — jusqu'à un montant mensuel par bénéficiaire. Au-delà, la part
 ...
 *
 * Le plafond ne dépend pas de la nature de la vente : le même calcul borne une assurance et un
 * carnet. Le parcours l'illustre sur une assurance, où le taux de 70 % rend visible ce que le
 * plafond change — la prise en charge cesse d'être une fraction de la vente pour devenir le
 * reliquat de droit, et les deux montants ne se confondent pas.
 *
 * Le parcours n'encaisse PAS : le plafonnement se lit avant la validation, et ne pas
 * finaliser laisse la consommation du bénéficiaire intacte — donc le parcours rejouable.
 */
scenario('VTE-50', async ({ etape, page }) => {
  ...
  await etape(1, async () => {
    ...
    // Rien ici n'annonce que le droit est presque épuisé : le taux affiché reste celui du
    // contrat. C'est la vente qui le révélera.
  });
  ...
  await etape(4, async () => {
    ...
    //
    // Le message nomme l'organisme par son libellé LONG et cite le montant PRIS EN CHARGE,
    // pas celui qui a été retiré. La nuance compte au comptoir : le caissier annonce ce que
    // l'organisme couvre, le client déduit le reste.
    await expect(contenu).toContainText(/plafonné à\s*15\s?000/i);
    await expect(contenu).toContainText('ASSURANCE MALADIE');
  });

  // ── Remise en état : la vente est abandonnée, la consommation du bénéficiaire inchangée. ──
