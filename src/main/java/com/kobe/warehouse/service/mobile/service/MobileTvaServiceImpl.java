...
public class MobileTvaServiceImpl implements MobileTvaService {
    ...
    public Tva getTva(MvtParam mvtParam) {
        ...
        if (nonNull(taxeWrapper)) {
            ...
            return new Tva(taxeWrapper.getTaxes().stream().map(tva -> new TvaItem(tva.getCodeTva(), NumberUtil.formatToString(tva.getMontantTtc()), NumberUtil.formatToString(tva.getMontantTaxe()), NumberUtil.formatToString(tva.getMontantHt()), (tva.getMontantTtc() * 100) / (double) totalTtc, null)).toList());
        }
        return null;
    }
