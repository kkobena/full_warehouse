...
export class GroupeTiersPayantService {
  ...
  async findPromise(): Promise<IGroupeTiersPayant> {
    return await this.http.get<IGroupeTiersPayant>(this.resourceUrl).toPromise();
  }

  async queryPromise(req?: any): Promise<IGroupeTiersPayant[]> {
    const options = createRequestOption(req);
    return await firstValueFrom(this.http
      .get<IGroupeTiersPayant[]>(this.resourceUrl, {params: options})
    );
  }
}
