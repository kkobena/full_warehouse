...
public class PaymentResource {
  ...
  public ResponseEntity<Payment> createPayment(@Valid @RequestBody Payment payment)
    ...
    if (payment.getId() != null) {
      throw new BadRequestAlertException(
          "A new payment cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     payment, or with status {@code 400 (Bad Request)} if the payment is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the payment couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/payments")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, payment.getId().toString()))
        .body(result);
  }
