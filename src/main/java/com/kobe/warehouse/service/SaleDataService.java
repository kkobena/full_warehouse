...
public class SaleDataService {
  ...
  public List<SaleDTO> allPrevente(String query, String type) {
    if (StringUtils.isEmpty(type) || type.equals(EntityConstant.TOUT))
      return this.allPreventes(query);
  ...
  public List<SaleDTO> allPreventeVNO(String query) {
    ...
    List<Predicate> predicates = new ArrayList<>();
    predicates.add(
        cb.equal(cashSaleRoot.get(CashSale_.user).get(User_.magasin), this.getUser().getMagasin()));
  ...
  public List<SaleLineDTO> getAllItemsBySaleId(Long id) {
    return this.salesLineRepository.findAllBySalesId(id).stream()
        .flatMap(List::stream)
        .map(SaleLineDTO::new)
  ...
  public List<PaymentDTO> getAllPaymentBySaleId(Long id) {
    return this.paymentRepository.findBySalesId(id).stream()
        .flatMap(List::stream)
        .map(PaymentDTO::new)
        .collect(Collectors.toList());
  }
  ...
  public List<TicketDTO> getAllTicketBySaleId(Long id) {
    return this.ticketRepository.findBySaleId(id).stream()
        .flatMap(List::stream)
        .map(TicketDTO::new)
        .collect(Collectors.toList());
  }
  ...
  private void periodeTimePredicat(
    ...
    if (StringUtils.isNotEmpty(fromHour) && StringUtils.isNotEmpty(toHour)) {
      predicates.add(
          cb.between(
              cb.function("TIME", Time.class, root.get(Sales_.updatedAt)),
              Time.valueOf(LocalTime.parse(fromHour)),
              Time.valueOf(LocalTime.parse(toHour.concat(":59")))));
    }
  }
  ...
  private ThirdPartySaleDTO buildFromEntity(ThirdPartySales thirdPartySales) {
    ...
    Pair<List<ThirdPartySaleLineDTO>, List<ClientTiersPayantDTO>> listListPair =
        buildTiersPayantDTOFromSale(
            this.thirdPartySaleLineRepository.findAllBySaleId(thirdPartySales.getId()));
  ...
  private SaleDTO buildSaleDTO(Sales s) {
    ...
    if (s instanceof ThirdPartySales) {
      ThirdPartySales thirdPartySales = (ThirdPartySales) s;
