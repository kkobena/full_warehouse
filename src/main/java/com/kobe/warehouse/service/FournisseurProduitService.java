...
public class FournisseurProduitService {
    ...
    public Optional<FournisseurProduitDTO> create(FournisseurProduitDTO dto) throws GenericError {
        ...
        if (constraint > 0)
            throw new GenericError("produit", "Le produit est déjà rattaché au fournisseur séléctionné", "duplicateProvider");
        ...
        if (constraint > 0)
            throw new GenericError("produit", String.format("Ce code %s est déjà associé à un produit pour ce grossiste", dto.getCodeCip()), "duplicateCodeCip");
        constraint = fournisseurProduitRepository.countByCodeCipAndProduitId(dto.getCodeCip(), dto.getProduitId());
        if (constraint > 0)
            throw new GenericError("produit", String.format("Ce code %s est déjà associé à un produit ", dto.getCodeCip()), "codeCip");
