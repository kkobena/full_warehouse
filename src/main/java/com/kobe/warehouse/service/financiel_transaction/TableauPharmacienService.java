...
public interface TableauPharmacienService extends MvtCommonService {
    String SALE_QUERY =
        ...
        SELECT %s,
               ...
               SUM(s.sales_amount) as montantTtc,SUM(p.paid_amount) as montantPaye, SUM(s.ht_amount) as montantHt,SUM(p.reel_amount) as reelAmount,
        ...
        FROM sales s LEFT JOIN  payment_transaction p  ON p.sale_id=s.id
