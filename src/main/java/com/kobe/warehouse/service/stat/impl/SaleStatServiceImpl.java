...
public class SaleStatServiceImpl implements SaleStatService {
    ...
    private List<Tuple> getGroupingByType(VenteRecordParamDTO venteRecordParamDTO, Pair<LocalDate, LocalDate> periode) {
        try {
    ...
    private List<Tuple> getCaGroupingByPeriode(VenteRecordParamDTO venteRecordParamDTO, Pair<LocalDate, LocalDate> periode) {
        try {
            return this.em.createNativeQuery(
                    buildPeriodeQuery(VenteStatQueryBuilder.PERIOQIQUE_CA_QUERY_GROUPING_BY_PERIODE, venteRecordParamDTO),
                    Tuple.class
                )
                .setParameter(1, java.sql.Date.valueOf(periode.getLeft()))
                .setParameter(2, java.sql.Date.valueOf(periode.getRight()))
    ...
    private List<VentePeriodeRecord> buildGroupingByPeriode(VenteRecordParamDTO venteRecordParamDTO) {
        ...
        if (!CollectionUtils.isEmpty(list)) {
            return list
                .stream()
                .map(e -> VenteStatQueryBuilder.buildVentePeriodeRecord(e, venteRecordParamDTO.getVenteStatGroupBy()))
                .toList();
        }
        return Collections.emptyList();
    }
    ...
    private List<Tuple> getModePaimentQuery(VenteRecordParamDTO venteRecordParamDTO) {
        try {
            Pair<LocalDate, LocalDate> periode = getPeriode(venteRecordParamDTO);
            return this.em.createNativeQuery(buildQuery(VenteStatQueryBuilder.MODE_PAIMENT, venteRecordParamDTO), Tuple.class)
                .setParameter(1, java.sql.Date.valueOf(periode.getLeft()))
                .setParameter(2, java.sql.Date.valueOf(periode.getRight()))
    ...
    private List<VenteModePaimentRecord> buildModePaimentQuery(VenteRecordParamDTO venteRecordParamDTO) {
