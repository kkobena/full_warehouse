...
export class CommandeService {
  ...
  fetchOrderLinesByCommandeId(commandeId: number): Observable<HttpResponse<IOrderLine[]>> {
    return this.http
      .get<IOrderLine[]>(`${this.resourceUrl}/pageable-order-lines/${commandeId}`, { params: { size: '99999' }, observe: 'response' })
      .pipe(map((res: HttpResponse<IOrderLine[]>) => this.convertOrderLineDateArrayFromServer(res)));
  }
  ...
  updateQuantityReceived(orderLine: IOrderLine): Observable<EntityResponseType> {
    return this.http
      .put<IOrderLine>(this.resourceUrl + '/update-order-line-quantity-received', orderLine, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  updateQuantityUG(orderLine: IOrderLine): Observable<EntityResponseType> {
    return this.http
      .put<IOrderLine>(this.resourceUrl + '/update-order-line-quantity-ug', orderLine, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  ...
  protected convertOrderLineDateFromClient(orderLine: IOrderLine): IOrderLine {
  ...
  protected convertOrderLineDateFromServer(res: HttpResponse<IOrderLine>): HttpResponse<IOrderLine> {
    if (res.body) {
