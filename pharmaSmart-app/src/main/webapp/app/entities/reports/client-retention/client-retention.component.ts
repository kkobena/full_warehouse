...
export default class ClientRetentionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      next: ({ kpi, list }) => {
        ...
        if (kpi.body?.totalClients) setTimeout(() => this.buildDoughnut(kpi.body!), 50);
      },
  ...
  protected segmentBadge(row: IClientRetentionRow): string {
    if ((row.joursAbsence ?? 0) <= 30) return 'bg-success-subtle text-success';
    if ((row.joursAbsence ?? 0) <= 90) return 'bg-warning-subtle text-warning';
    return 'bg-danger-subtle text-danger';
  }

  protected segmentLabel(row: IClientRetentionRow): string {
    if ((row.joursAbsence ?? 0) <= 30) return 'Actif';
    if ((row.joursAbsence ?? 0) <= 90) return 'À risque';
    return 'Perdu';
  }
