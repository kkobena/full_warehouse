...
public class AjustementResource {
    ...
    public ResponseEntity<List<AjustDTO>> getAllAjustts(@RequestParam(value = "search", required = false) String search,
                                                        @RequestParam(name = "fromDate", required = false ,defaultValue = "2020-01-01")
                                                            LocalDate fromDate, @RequestParam(name = "toDate", required = false,defaultValue = "2030-01-01")
                                                            LocalDate toDate,
                                                        Pageable pageable) {
    ...
    public ResponseEntity<Void> delete(@PathVariable Long id) {
        ...
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, "ajustement", id.toString())).build();
    }
}
