...
public class DashboardCARepository {
    ...
    public long getDetteFournisseur() {
        Number result = (Number) entityManager.createNativeQuery(DETTE_FOURNISSEUR_SQL)
            .getSingleResult();
    ...
    public long getNbEcheancesEnRetard() {
        Number result = (Number) entityManager.createNativeQuery(NB_ECHEANCES_RETARD_SQL)
            .getSingleResult();
        return result != null ? result.longValue() : 0L;
    }

    public long getNbFacturesImpayees() {
        Number result = (Number) entityManager.createNativeQuery(NB_FACTURES_IMPAYEES_SQL)
            .getSingleResult();
        return result != null ? result.longValue() : 0L;
    }
    ...
    @SuppressWarnings("unchecked")
    public List<Object[]> findRemisesTopProducts(LocalDate startDate, LocalDate endDate,
        int limit) {
