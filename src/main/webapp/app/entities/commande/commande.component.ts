...
export class CommandeComponent implements OnInit {
  ...
  protected onSuggestionTabChange(tab: string): void {
    this.suggestionActiveTab = tab;
  }

  // Méthodes SEMOIS
  protected onSemoisFilterChange(): void {
  ...
  // Getters/Setters pour les filtres SEMOIS (pour binding bidirectionnel)
  protected get semoisSelectedClasse(): ClasseCriticite | null {
    return this.suggestion()?.getSemoisComponent()?.selectedClasse() ?? null;
  }

  protected set semoisSelectedClasse(value: ClasseCriticite | null) {
    const semoisComp = this.suggestion()?.getSemoisComponent();
    if (semoisComp) {
      semoisComp.selectedClasse.set(value);
    }
  }

  protected get semoisUrgenceFilter(): string | null {
    return this.suggestion()?.getSemoisComponent()?.urgenceFilter() ?? null;
  }

  protected set semoisUrgenceFilter(value: string | null) {
    const semoisComp = this.suggestion()?.getSemoisComponent();
    if (semoisComp) {
      semoisComp.urgenceFilter.set(value);
    }
  }

  protected get semoisSearchText(): string {
    return this.suggestion()?.getSemoisComponent()?.searchText() ?? '';
  }

  protected set semoisSearchText(value: string) {
    const semoisComp = this.suggestion()?.getSemoisComponent();
    if (semoisComp) {
