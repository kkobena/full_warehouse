...
public class CommandServiceImpl implements CommandService {
    ...
    public Commande createOrUpdateOrderLine(OrderLineDTO orderLineDTO) {
        ...
        if (orderLine == null) {
            ...
            orderLine.setOrderAmount(
                orderLine.getOrderUnitPrice() * orderLine.getQuantityRequested());
            orderLine.setGrossAmount(
                orderLine.getOrderCostAmount() * orderLine.getQuantityRequested());
        }
    ...
    public void updateOrderLineQuantityReceived(OrderLineDTO orderLineDTO) {
        orderLineService
            ...
            .ifPresentOrElse(
                orderLine ->
                    orderLineService.updateOrderLineQuantityReceived(
                        orderLine, orderLineDTO.getQuantityReceived()),
    ...
    public void updateOrderLineQuantityUg(OrderLineDTO orderLineDTO) {
        orderLineService
            ...
            .ifPresentOrElse(
                orderLine ->
                    orderLineService.updateOrderLineQuantityUG(
                        orderLineDTO.getId(), orderLineDTO.getUgQuantity()),
                () -> new NullPointerException());
    }
            ...
            .ifPresent(
                orderLine -> {
                    ...
                    updateCommandeAmount(
                        commande, orderLine.getGrossAmount() * (-1),
    ...
    public VerificationResponseCommandeDTO importerReponseCommande(
        ...
        if (extension.equalsIgnoreCase(CSV)) {
            return verificationCommandeCsv(
                multipartFile, commandeRepository.getReferenceById(commandeId));
        }
        return verificationCommandeExcel(
            multipartFile, commandeRepository.getReferenceById(commandeId));
    }
    ...
    private CommandeResponseDTO uploadCSVFormat(
        ...
        switch (commandeModel) {
            case LABOREX:
                commandeResponseDTO =
                    uploadLaborexCSVFormat(commande, multipartFile, items, longOrderLineMap,
                        fournisseurId);
                break;
            case COPHARMED:
                commandeResponseDTO =
            ...
            case DPCI:
                commandeResponseDTO =
                    uploadDPCICSVFormat(commande, multipartFile, items, longOrderLineMap,
                        fournisseurId);
                break;
            case TEDIS:
                commandeResponseDTO =
                    uploadTEDISCSVFormat(commande, multipartFile, items, longOrderLineMap,
                        fournisseurId);
                break;
            default:
                throw new GenericError(
        ...
        createRuptureFile(commande.getOrderRefernce(), commandeModel,
            commandeResponseDTO.getItems());
        ...
        try (BufferedReader reader =
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (fournisseurProduitOptional.isPresent()) {
                    ...
                    int currentStock =
                        orderLineService.produitTotalStockWithQantitUg(
                    ...
                    findInCommandeMap(longOrderLineMap, fournisseurProduit.getId())
                        .ifPresentOrElse(
                            orderLine -> {
                                ...
                                updateInRecord(
                                    commande, orderLine, quantityReceived, 0, oldQty, oldTaxAmount,
                                    0);
                            },
                            () ->
                                createInRecord(
        ...
        try (CSVParser parser =
            ...
            for (CSVRecord record : parser) {
                if (totalItemCount > 0) {
                    ...
                    Optional<FournisseurProduit> fournisseurProduitOptional =
                        orderLineService.getFournisseurProduitByCriteria(codeProduit,
                            fournisseurId);
                    if (fournisseurProduitOptional.isPresent()) {
                        FournisseurProduit fournisseurProduit = fournisseurProduitOptional.get();
                        int currentStock =
    ...
    private OrderLineDTO buildOrderLineDTOFromRecord(
        ...
        orderLineDTO.setOrderUnitPrice(
            orderUnitPrice > 0 ? orderUnitPrice : fournisseurProduit.getPrixUni());
        orderLineDTO.setOrderCostAmount(
    ...
    private Optional<OrderLine> findOrderLineInSetOrderLine(
        ...
        return orderLines.stream()
            .filter(
                orderLine ->
                    orderLine.getFournisseurProduit()
                        .equals(orderLineToCheck.getFournisseurProduit()))
            .findFirst();
    }
