...
public class FormProduitResource {
   ...
   *     formProduitDTO, or with status {@code 400 (Bad Request)} if the formProduit has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/form-produits")
  public ResponseEntity<FormProduitDTO> createFormProduit(
      @Valid @RequestBody FormProduitDTO formProduitDTO) throws URISyntaxException {
    log.debug("REST request to save FormProduit : {}", formProduitDTO);
    if (formProduitDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new formProduit cannot already have an ID", ENTITY_NAME, "idexists");
    }
    FormProduitDTO result = formProduitService.save(formProduitDTO);
    return ResponseEntity.created(new URI("/api/form-produits/" + result.getId()))
        .headers(
   ...
   *     formProduitDTO, or with status {@code 400 (Bad Request)} if the formProduitDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the formProduitDTO couldn't be
   *     updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/form-produits")
  public ResponseEntity<FormProduitDTO> updateFormProduit(
      @Valid @RequestBody FormProduitDTO formProduitDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, formProduitDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of formProduits in
   *     body.
   */
