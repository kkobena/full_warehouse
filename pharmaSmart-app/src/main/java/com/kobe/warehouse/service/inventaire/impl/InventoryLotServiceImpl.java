...
public class InventoryLotServiceImpl implements InventoryLotService {
    ...
    public InventoryLotRecord save(InventoryLotRecord record) {
        ...
        // La création avec quantité comptée doit aussi resynchroniser la ligne parente
        // (sinon seule la mise à jour d'un lot le faisait — écart mobile/web)
        if (entity.getQuantityOnHand() != null) {
            syncParentLineQuantity(line.getId());
        }
    ...
    public void delete(Long inventoryLotId) {
        Long parentLineId = inventoryLotRepository.findById(inventoryLotId)
            .map(lot -> lot.getStoreInventoryLine().getId())
            .orElse(null);
        inventoryLotRepository.deleteById(inventoryLotId);
        if (parentLineId != null) {
            inventoryLotRepository.flush();
            // La quantité de la ligne parente = somme des lots restants
            syncParentLineQuantity(parentLineId);
        }
    }
    ...
    private void syncParentLineQuantity(Long storeInventoryLineId) {
        ...
        line.setCountedBy(userService.getUser());
