...
public class MobileCasseRecapServiceImpl implements MobileCasseRecapService {
    ...
    private List<ListItem> buildItems(List<TicketZData> data) {
        ...
        return data.stream().map(d -> new ListItem(d.libelle(), d.montant(), null)).toList();
    }
    ...
    private List<UserCaisseRecap> buildUserRecap(List<TicketZRecap> data) {
        ...
        return data.stream().map(d -> new UserCaisseRecap(d.userName(), buildItems(d.datas()), buildItems(d.summary()))).toList();
    }
