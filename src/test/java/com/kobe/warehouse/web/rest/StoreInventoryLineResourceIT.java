...
public class StoreInventoryLineResourceIT {
  ...
  @Autowired private StoreInventoryLineRepository storeInventoryLineRepository;

  @Autowired private EntityManager em;

  @Autowired private MockMvc restStoreInventoryLineMockMvc;
  ...
  public void createStoreInventoryLine() throws Exception {
    ...
    restStoreInventoryLineMockMvc
        .perform(
            post("/api/store-inventory-lines")
                .with(csrf())
    ...
    StoreInventoryLine testStoreInventoryLine =
        storeInventoryLineList.get(storeInventoryLineList.size() - 1);
    ...
    assertThat(testStoreInventoryLine.getInventoryValueCost())
        .isEqualTo(DEFAULT_INVENTORY_VALUE_COST);
  }
  ...
  public void getAllStoreInventoryLines() throws Exception {
    ...
    restStoreInventoryLineMockMvc
        .perform(get("/api/store-inventory-lines?sort=id,desc"))
        ...
        .andExpect(
            jsonPath("$.[*].inventoryValueCost").value(hasItem(DEFAULT_INVENTORY_VALUE_COST)))
        .andExpect(
            jsonPath("$.[*].inventoryValueLatestSellingPrice")
                .value(hasItem(DEFAULT_INVENTORY_VALUE_LATEST_SELLING_PRICE)));
  }
  ...
  public void getStoreInventoryLine() throws Exception {
    ...
    restStoreInventoryLineMockMvc
        .perform(get("/api/store-inventory-lines/{id}", storeInventoryLine.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        ...
        .andExpect(
            jsonPath("$.inventoryValueLatestSellingPrice")
                .value(DEFAULT_INVENTORY_VALUE_LATEST_SELLING_PRICE));
  }
  ...
  public void getNonExistingStoreInventoryLine() throws Exception {
    // Get the storeInventoryLine
    restStoreInventoryLineMockMvc
        .perform(get("/api/store-inventory-lines/{id}", Long.MAX_VALUE))
  ...
  public void updateStoreInventoryLine() throws Exception {
    ...
    StoreInventoryLine updatedStoreInventoryLine =
        storeInventoryLineRepository.findById(storeInventoryLine.getId()).get();
    // Disconnect from session so that the updates on updatedStoreInventoryLine are not directly
    // saved in db
    ...
    assertThat(testStoreInventoryLine.getInventoryValueCost())
        .isEqualTo(UPDATED_INVENTORY_VALUE_COST);
  }
  ...
  public void deleteStoreInventoryLine() throws Exception {
    ...
    restStoreInventoryLineMockMvc
        .perform(
            delete("/api/store-inventory-lines/{id}", storeInventoryLine.getId())
                .with(csrf())
