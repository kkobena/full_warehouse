...
public class ProduitDTO {
    ...
    public ProduitDTO(Produit produit, Magasin magasin, StockProduit stockProduitPointOfSale) {
        ...
        if (parent != null) {
            produitId = parent.getId();
            produitLibelle = parent.getLibelle();
        }
        produits =
            produit.getProduits().stream().map(ProduitDTO::new).collect(Collectors.toList());
        fournisseurProduits =
            produit.getFournisseurProduits().stream()
                .map(FournisseurProduitDTO::new)
                .collect(Collectors.toSet());
        chiffre = produit.getChiffre();
        createdAt = produit.getCreatedAt();
        prixMnp = produit.getPrixMnp();
        FournisseurProduit fournisseurProduitPrincipal = produit.getFournisseurProduitPrincipal();
        if (fournisseurProduitPrincipal != null) {
            codeCip = fournisseurProduitPrincipal.getCodeCip();
            fournisseurProduit = new FournisseurProduitDTO(fournisseurProduitPrincipal);
            fournisseurId = fournisseurProduit.getFournisseurId();
            costAmount = fournisseurProduitPrincipal.getPrixAchat();
            regularUnitPrice = fournisseurProduitPrincipal.getPrixUni();
        }
        Laboratoire laboratoire = produit.getLaboratoire();
        if (laboratoire != null) {
            laboratoireId = laboratoire.getId();
            laboratoireLibelle = laboratoire.getLibelle();
        }
        FormProduit formProduit = produit.getForme();
        if (formProduit != null) {
            formeId = formProduit.getId();
            formeLibelle = formProduit.getLibelle();
        }
        TypeEtiquette typeEtiquette = produit.getTypeEtyquette();
        if (typeEtiquette != null) {
            typeEtyquetteId = typeEtiquette.getId();
            typeEtyquetteLibelle = typeEtiquette.getLibelle();
        }
        FamilleProduit familleProduit = produit.getFamille();
        if (familleProduit != null) {
            familleId = familleProduit.getId();
            familleLibelle = familleProduit.getLibelle();
        }
        GammeProduit gammeProduit = produit.getGamme();
        if (gammeProduit != null) {
            gammeId = gammeProduit.getId();
            gammeLibelle = gammeProduit.getLibelle();
        }
        ...
        stockProduits =
            produit.getStockProduits().stream()
                .filter(s -> s.getStorage().getMagasin().getId().equals(magasin.getId()))
                .map(StockProduitDTO::new)
                .collect(Collectors.toSet());
        if (stockProduitPointOfSale != null) {
            stockProduit = new StockProduitDTO(stockProduitPointOfSale);
            saleOfPointStock = stockProduitPointOfSale.getQtyStock();
        }
        rayonProduits =
            produit.getRayonProduits().stream().map(RayonProduitDTO::new).collect(Collectors.toList());
        Optional<RayonProduitDTO> rayon =
            rayonProduits.stream()
                .filter(
                    r ->
                        (r.getStorageType().equalsIgnoreCase(StorageType.PRINCIPAL.getValue())
                            && r.getMagasinId().equals(magasin.getId())))
                .findFirst();

        if (rayon.isPresent()) {
            RayonProduitDTO rayonProduitDTO = rayon.get();
            rayonId = rayonProduitDTO.getRayonId();
            rayonLibelle = rayonProduitDTO.getLibelleRayon();
        }
     /* if(!CollectionUtils.isEmpty(produit.getParcoursProduits())){
          this.parcoursProduits = produit.getParcoursProduits().stream().collect(Collectors.toList());
      }*/
        ...
        totalQuantity =
            stockProduits.stream().collect(Collectors.summingInt(StockProduitDTO::getQtyStock));
        qtyAppro = produit.getQtyAppro();
        qtySeuilMini = produit.getQtySeuilMini();
        dateperemption = produit.getCheckExpiryDate();
        chiffre = produit.getChiffre();
        deconditionnable = produit.getDeconditionnable();
        codeEan = produit.getCodeEan();
        RemiseProduit remiseProduit = produit.getRemise();
        if (remiseProduit != null) {
            remiseId = remiseProduit.getId();
            tauxRemise = remiseProduit.getRemiseValue();
        }
        perimeAt = produit.getPerimeAt();
        if (produit.getPerimeAt() != null) {
            expirationDate = produit.getPerimeAt().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
        }
        status = produit.getStatus().ordinal();
    }
    ...
    public ProduitDTO(Produit produit) {
        ...
        FournisseurProduit fournisseurProduitPrincipal = produit.getFournisseurProduitPrincipal();
        System.out.println(
            "fournisseurProduitPrincipal ==+++++++++++++++++" + fournisseurProduitPrincipal);
        if (fournisseurProduitPrincipal != null) {
            codeCip = fournisseurProduitPrincipal.getCodeCip();
            fournisseurProduit = new FournisseurProduitDTO(fournisseurProduitPrincipal);
            fournisseurId = fournisseurProduit.getFournisseurId();
    ...
    public static ProduitDTO lite(Produit produit) {
        ...
        //  dto.setParcoursProduits(produit.getParcoursProduits().stream().collect(Collectors.toList()));
        return dto;
    }
    ...
    public ProduitDTO setDailyStocks(List<DailyStock> dailyStocks) {
