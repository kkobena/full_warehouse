...
public class TvaResource {
   ...
   *     tvaDTO, or with status {@code 400 (Bad Request)} if the tva has already an ID.
  ...
  public ResponseEntity<TvaDTO> createTva(@Valid @RequestBody TvaDTO tvaDTO)
    ...
    if (tvaDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new tva cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the tvaDTO, or
