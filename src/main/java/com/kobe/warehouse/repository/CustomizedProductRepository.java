...
public class CustomizedProductRepository implements CustomizedProductService {
    ...
    public StockProduit updateTotalStock(Produit produit, int stockIn, int stockUg) {
        ...
        System.err.print(stockIn + " stockIn ");
        System.err.print(stockUg + " stockUg ");
        ...
        System.err.print(stockProduit.getQtyStock() + "  getQtyStock ");
        System.err.print(stockProduit.getQtyUG() + " getQtyUG ");
        System.err.print(stockProduit.getTotalStockQuantity() + " getTotalStockQuantity ");
        System.err.println(" ");
    ...
    public List<ProduitDTO> findAll(ProduitCriteria produitCriteria) throws Exception {
        ...
        return q.getResultList().stream().map(ProduitBuilder::fromProduit)
                    ...
                    p -> {
                        ...
                        if (detailsInventaire != null) {
                            dto.setLastInventoryDate(
                                detailsInventaire.getStoreInventory().getUpdatedAt());
                        }
        ...
        try {
            ...
            predicates.add(
                cb.or(
                    ...
                    cb.like(root.get(FournisseurProduit_.produit).get(Produit_.codeEan),
                        criteteria)));
            predicates.add(
    ...
    private List<Predicate> produitPredicate(
        ...
        if (produitCriteria.getStorageId() != null || produitCriteria.getRayonId() != null) {
            ...
            if (produitCriteria.getStorageId() != null) {
                predicates.add(
                    cb.equal(
                        st.get(StockProduit_.storage).get(Storage_.id),
            ...
            if (produitCriteria.getRayonId() != null) {
                SetJoin<Produit, RayonProduit> rp = root.joinSet(Produit_.RAYON_PRODUITS,
                    JoinType.INNER);
                predicates.add(
                    cb.equal(rp.get(RayonProduit_.rayon).get(Rayon_.id),
        ...
        if (produitCriteria.getLaboratoireId() != null) {
            ...
            predicates.add(
                cb.equal(laboratoireIdJoin.get(Laboratoire_.id),
