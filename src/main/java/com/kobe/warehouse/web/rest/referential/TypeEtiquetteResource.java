...
public class TypeEtiquetteResource {
   ...
   *     typeEtiquetteDTO, or with status {@code 400 (Bad Request)} if the typeEtiquette has already
   *     an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/type-etiquettes")
  public ResponseEntity<TypeEtiquetteDTO> createTypeEtiquette(
      @Valid @RequestBody TypeEtiquetteDTO typeEtiquetteDTO) throws URISyntaxException {
    log.debug("REST request to save TypeEtiquette : {}", typeEtiquetteDTO);
    if (typeEtiquetteDTO.getId() != null) {
      throw new BadRequestAlertException(
          "A new typeEtiquette cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     typeEtiquetteDTO, or with status {@code 400 (Bad Request)} if the typeEtiquetteDTO is not
   *     valid, or with status {@code 500 (Internal Server Error)} if the typeEtiquetteDTO couldn't
   *     be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/type-etiquettes")
  public ResponseEntity<TypeEtiquetteDTO> updateTypeEtiquette(
      @Valid @RequestBody TypeEtiquetteDTO typeEtiquetteDTO) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, typeEtiquetteDTO.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of typeEtiquettes
   *     in body.
   */
   ...
   *     typeEtiquetteDTO, or with status {@code 404 (Not Found)}.
   */
