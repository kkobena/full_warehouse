...
ALTER TABLE warehouse.lot
    ADD COLUMN IF NOT EXISTS serial_number VARCHAR(50) NULL;

COMMENT ON COLUMN warehouse.lot.serial_number IS 'Numéro de série FMD (AI 21 GS1 DataMatrix). NULL = non présent ou scan 1D.';

CREATE UNIQUE INDEX IF NOT EXISTS lot_serial_number_produit_idx
    ON warehouse.lot (serial_number, produit_id)
