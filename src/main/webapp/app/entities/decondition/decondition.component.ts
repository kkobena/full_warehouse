...
export class DeconditionComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.deconditionService
      ...
      .subscribe(
        (res: HttpResponse<IDecondition[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInDeconditions();
  }
  ...
  protected onSuccess(data: IDecondition[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/decondition'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
