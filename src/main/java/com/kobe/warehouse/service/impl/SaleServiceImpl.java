...
public class SaleServiceImpl extends SaleCommonService implements SaleService {
  ...
  public SaleLineDTO updateSaleLine(SaleLineDTO saleLine) {
    SalesLine salesLine = this.salesLineService.getOneById(saleLine.getId());
    int oldAmont = salesLine.getSalesAmount();
    int oldQty = salesLine.getQuantitySold();
    this.salesLineService.updateSaleLine(saleLine, salesLine);
  ...
  public Payment buildPaymentFromDTO(PaymentDTO dto, Sales s) {
    ...
    PaymentMode paymentMode =
        paymentModeRepository
            ...
            .orElse(paymentModeRepository.getOne("CASH"));
  ...
  public CashSaleDTO createCashSale(CashSaleDTO dto) {
    ...
    UninsuredCustomer uninsuredCustomer =
        dto.getCustomer() != null
            ? uninsuredCustomerRepository.getOne(dto.getCustomer().getId())
            : null;
    CashSale c = new CashSale();
  ...
  private SalesLine createOrUpdateSaleLine(SaleLineDTO dto) {
    ...
    SalesLine salesLine =
        this.salesLineService.create(
            dto, storageId, this.cashSaleRepository.getOne(dto.getSaleId()));
  ...
  public ResponseDTO save(CashSaleDTO dto)
    ...
    User user = this.storageService.getUser();
    this.cashRegisterService.checkIfCashRegisterIsOpen(user, this.storageService.getSystemeUser());
    ...
    buildReference(p);
    Ticket ticket = this.ticketService.buildTicket(dto, p, user, buildTvaData(p.getSalesLines()));
  ...
  public ResponseDTO putCashSaleOnHold(CashSaleDTO dto) {
    ...
    this.paymentService.buildPaymentFromFromPaymentDTO(cashSale, dto, user);
