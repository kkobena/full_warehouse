...
public class TableauPharmacienProxy {
  ...
  public ResponseEntity<List<GroupeFournisseurDTO>> fetchGroupGrossisteToDisplay() {
    ...
    public ResponseEntity<Resource> exportToPdf( HttpServletRequest request,
        @RequestParam(value = "fromDate", required = false) LocalDate fromDate,
        @RequestParam(value = "toDate", required = false) LocalDate toDate,
        @RequestParam(value = "categorieChiffreAffaires", required = false)
        Set<CategorieChiffreAffaire> categorieChiffreAffaires,
        ...
        Resource resource= tableauPharmacienService.exportToPdf(new MvtParam(fromDate, toDate, categorieChiffreAffaires, statuts, typeVentes, groupeBy)
            .build());
