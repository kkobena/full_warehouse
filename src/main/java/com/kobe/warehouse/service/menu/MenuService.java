...
public class MenuService {
  ...
  public Set<MenuDTO> getConnectedUserMenus() {
    ...
    Map<Boolean, List<MenuSpecialisation>> partitioned =
        menuSpecialisations.stream().collect(Collectors.partitioningBy(MenuSpecialisation::isRoot));
        ...
        root -> {
          ...
          menu.setMenus(
              children.stream()
                  .filter(
                      menuSpecialisation ->
                          menuSpecialisation.getParent() != null
                              && menuSpecialisation.getParent().equals(root.getId()))
                  .map(
                      menuSpecialisation -> {
