...
public class OrderLineServiceImpl implements OrderLineService {
  ...
  public OrderLine buildOrderLineFromOrderLineDTO(OrderLineDTO orderLineDTO) {
    ...
    Optional<FournisseurProduit> fournisseurProduitOptional =
        fournisseurProduitService.findFirstByProduitIdAndFournisseurId(
            orderLineDTO.getProduitId(), orderLineDTO.getCommande().getFournisseurId());
  ...
  private void removeProductState(Produit produit) {
    List<ProductState> productStates =
        this.productStateService.fetchByProduitAndState(
            produit, ProductStateEnum.COMMANDE_EN_COURS);
    if (!CollectionUtils.isEmpty(productStates)) {
      if (productStates.size() == 1) {
        productStates.forEach(this.productStateService::remove);
      } else {
  ...
  public Optional<OrderLine> findOneFromCommande(
    ...
    Optional<FournisseurProduit> fournisseurProduitOptional =
        fournisseurProduitService.findFirstByProduitIdAndFournisseurId(produitId, fournisseurId);
  ...
  public void updateRequestedLineToPassedLine(Set<OrderLine> orderLines) {
    ...
    orderLines.forEach(
        orderLine -> {
          removeProductState(orderLine.getFournisseurProduit().getProduit());
          this.productStateService.addState(
              orderLine.getFournisseurProduit().getProduit(), ProductStateEnum.COMMANDE_PASSE);
        });
  }
  ...
  public List<FournisseurProduit> getFournisseurProduitsByFournisseur(Long founisseurId) {
    return fournisseurProduitService.getFournisseurProduitsByFournisseur(
        founisseurId, PageRequest.of(0, 1000));
  }
