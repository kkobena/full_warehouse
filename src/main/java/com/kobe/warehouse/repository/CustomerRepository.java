...
public interface CustomerRepository extends JpaRepository<Customer, Long>, JpaSpecificationExecutor<Customer> {
    ...
    default Specification<Customer> specialisationQueryString(String queryValue) {
        return (root, query, cb) ->
            cb.or(cb.like(cb.upper(root.get(Customer_.firstName)), queryValue), cb.like(cb.upper(root.get(Customer_.lastName)), queryValue), cb.like(cb.upper(root.get(Customer_.code)), queryValue),
                cb.like(cb.upper(cb.concat(cb.concat(root.get(Customer_.firstName), " "), root.get(Customer_.lastName))), queryValue), cb.like(cb.upper(root.get(Customer_.phone)), queryValue)
            );
    }
    ...
    default Specification<Customer> specialisationCheckExist(String firstName, String lastName, String phone) {
        return (root, query, cb) -> cb.and(cb.equal(cb.upper(root.get(Customer_.firstName)), firstName), cb.equal(cb.upper(root.get(Customer_.lastName)), lastName), cb.equal(cb.upper(root.get(Customer_.phone)), phone));
    }
