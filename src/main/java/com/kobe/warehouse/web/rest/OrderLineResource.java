...
public class OrderLineResource {
  ...
  public ResponseEntity<OrderLine> createOrderLine(@Valid @RequestBody OrderLine orderLine)
      throws URISyntaxException {
    log.debug("REST request to save OrderLine : {}", orderLine);
    if (orderLine.getId() != null) {
      throw new BadRequestAlertException(
          "A new orderLine cannot already have an ID", ENTITY_NAME, "idexists");
    }
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, orderLine.getId().toString()))
        .body(result);
  }
  ...
  public ResponseEntity<List<OrderLine>> getAllOrderLines(@PathVariable Long id) {
    ...
    List<OrderLine> listOrderLines =
        orderLineRepository.findByCommandeIdOrderByFournisseurProduitProduitLibelleAsc(id);
