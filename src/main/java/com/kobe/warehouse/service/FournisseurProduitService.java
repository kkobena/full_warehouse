...
public class FournisseurProduitService {
    ...
    public Optional<FournisseurProduitDTO> create(FournisseurProduitDTO dto) throws GenericError {
        ...
        return Optional.ofNullable(createNewFournisseurProduit(dto))
    ...
    public Optional<FournisseurProduitDTO> update(FournisseurProduitDTO dto) throws Exception {
        long constraint;
        FournisseurProduit fournisseurProduit = fournisseurProduitRepository.getReferenceById(
            dto.getId());
        if (!dto.getCodeCip().equals(fournisseurProduit.getCodeCip())) {
            ...
            if (constraint > 0) {
                throw new GenericError(
                    "produit",
                    String.format(
                        "Ce code %s est déjà associé à un produit pour ce grossiste",
                        dto.getCodeCip()),
                    "duplicateCodeCip");
            }
            constraint =
    ...
    public void updateCip(String cip, FournisseurProduit fournisseurProduit) {
        ...
        if (constraint > 0) {
            throw new GenericError(
                "produit", String.format("Ce code %s est déjà associé à un produit ", cip),
                "codeCip");
        }
