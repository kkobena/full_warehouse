...
import VieillissementCreancesComponent
  from '../vieillissement-creances/vieillissement-creances.component';
...
import ProfitabilityAnalysisComponent
  from '../profitability-analysis/profitability-analysis.component';
...
import VieillissementDifferesComponent
  from '../vieillissement-differes/vieillissement-differes.component';
...
@Component({
  ...
  imports: [NavSidebarComponent, NavSectionLinkComponent, CommonModule, NgbNavModule,
...
export default class FinanceReportsComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.active() === 'pnl-analytique' && !this.showPnlAnalytique()) {
      ...
      } else if (this.showCreances()) {
        this.active.set('creances');
      } else if (this.showCashFlowBfr()) {
        this.active.set('cash-flow-bfr');
      } else if (this.showSituationCreances()) {
        this.active.set('situation-creances');
      } else if (this.showVieillissementDiff()) {
        this.active.set('vieillissement-differes');
      } else if (this.showAvoirsAnalytics()) {
        this.active.set('avoirs-analytics');
      } else if (this.showTauxRecouvrement()) {
        this.active.set('taux-recouvrement-tp');
      }
    }
  }
}
