...
import static com.kobe.warehouse.web.rest.AccountResourceIT.TEST_USER_LOGIN;
...
class AccountResourceIT {
  ...
  void testChangePasswordWrongExistingPassword() throws Exception {
    ...
    User updatedUser =
        userRepository.findOneByLogin("change-password-wrong-existing-password").orElse(null);
    ...
    restAccountMockMvc
        .perform(
            post("/api/account/change-password")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    TestUtil.convertObjectToJsonBytes(new PasswordChangeDTO(currentPassword, "")))
                .with(csrf()))
        .andExpect(status().isBadRequest());
  ...
  void testGetCurrentSessions() throws Exception {
    ...
    restAccountMockMvc
        ...
        .andExpect(
            jsonPath("$.[*].tokenDate")
                .value(hasItem(containsString(token.getTokenDate().toString()))));
  }
  ...
  void testInvalidateSession() throws Exception {
    ...
    restAccountMockMvc
        .perform(delete("/api/account/sessions/invalidate-session").with(csrf()))
        .andExpect(status().isOk());
  ...
  void testFinishPasswordReset() throws Exception {
    ...
    assertThat(passwordEncoder.matches(keyAndPassword.getNewPassword(), updatedUser.getPassword()))
        .isTrue();
  }
