...
@Component({
  ...
  imports: [
    ...
    InputText,
    NgxSpinnerModule,
    KeyFilter, ProduitAutocompleteComponent, ToolbarModule, QuantiteProdutSaisieComponent, CtaComponent, ConfirmDialogComponent, ToastAlertComponent, DecimalPipe, TableModule, Tag, RemoveButtonTextComponent, BackButtonComponent, ButtonGroup, IconField, InputIcon, DatePickerComponent
  ],
...
export class AjoutPerimesComponent implements OnInit {
  ...
  onRemoveItem(item: ProductToDestroy): void {
    this.confimDialog().onConfirm(
      ...
      'Confirmation',
      'Êtes-vous sûr de vouloir supprimer cet article ?', null,
      () => {
  ...
  private removeItem(item: ProductToDestroy): void {
    this.productToDestroyService.delete({
      ids: [item.id],
    }).subscribe({
      next: () => {
        this.loadPage();
  ...
  protected modifyProductQuantity(item: ProductToDestroy, inputValue: any): void {
    this.isSaving = true;
    this.productToDestroyService.modifyProductQuantity({
      id: item.id,
