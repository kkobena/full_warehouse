...
public class InventoryTransactionResourceIT {
  ...
  @Autowired private MockMvc restInventoryTransactionMockMvc;
  ...
  public void createInventoryTransaction() throws Exception {
    ...
    InventoryTransaction testInventoryTransaction =
        inventoryTransactionList.get(inventoryTransactionList.size() - 1);
  ...
  public void getAllInventoryTransactions() throws Exception {
    ...
    restInventoryTransactionMockMvc
        .perform(get("/api/inventory-transactions?sort=id,desc"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(jsonPath("$.[*].id").value(hasItem(inventoryTransaction.getId().intValue())))
        .andExpect(
            jsonPath("$.[*].transactionType").value(hasItem(DEFAULT_TRANSACTION_TYPE.toString())))
  ...
  public void getInventoryTransaction() throws Exception {
    ...
    restInventoryTransactionMockMvc
        .perform(get("/api/inventory-transactions/{id}", inventoryTransaction.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
  ...
  public void getNonExistingInventoryTransaction() throws Exception {
    // Get the inventoryTransaction
    restInventoryTransactionMockMvc
        .perform(get("/api/inventory-transactions/{id}", Long.MAX_VALUE))
  ...
  public void updateInventoryTransaction() throws Exception {
    ...
    InventoryTransaction updatedInventoryTransaction =
        inventoryTransactionRepository.findById(inventoryTransaction.getId()).get();
    // Disconnect from session so that the updates on updatedInventoryTransaction are not directly
    // saved in db
  ...
  public void deleteInventoryTransaction() throws Exception {
    ...
    restInventoryTransactionMockMvc
        .perform(
            delete("/api/inventory-transactions/{id}", inventoryTransaction.getId())
                .with(csrf())
