...
public interface InventaireService {
  ...
  default String buildBaseQuery(
      String baseQuery, StoreInventoryLineFilterRecord storeInventoryLineFilterRecord) {
    if (Objects.nonNull(storeInventoryLineFilterRecord.storageId())
        || Objects.nonNull(storeInventoryLineFilterRecord.rayonId())) {
    ...
    } else return baseQuery.replace("{join_statement}", "").replace("{join_statement_where}", "");
  }
  ...
  default String buildFetchDetailQuery(
    ...
    return buildFetchDetailQuery(
        StoreInventoryLineFilterBuilder.BASE_QUERY, storeInventoryLineFilterRecord);
  }

  default String buildFetchDetailQueryCount(
      StoreInventoryLineFilterRecord storeInventoryLineFilterRecord) {
    return buildFetchDetailQuery(
        StoreInventoryLineFilterBuilder.COUNT, storeInventoryLineFilterRecord);
  }
  ...
  default String buildExportQuery(StoreInventoryExportRecord inventoryExportRecord) {
    StoreInventoryLineFilterRecord storeInventoryLineFilterRecord =
        inventoryExportRecord.filterRecord();
    String whereClose = "";
    if (Objects.nonNull(storeInventoryLineFilterRecord.rayonId())) {
      whereClose =
          whereClose.concat(
              String.format(
                  StoreInventoryLineFilterBuilder.EXPORT_RAYON_CLOSE_QUERY,
                  storeInventoryLineFilterRecord.rayonId()));
    }
    if (Objects.nonNull(storeInventoryLineFilterRecord.storageId())) {
      whereClose =
          whereClose.concat(
              String.format(
                  StoreInventoryLineFilterBuilder.EXPORT_STORAGE_CLOSE_QUERY,
                  storeInventoryLineFilterRecord.storageId()));
    }
