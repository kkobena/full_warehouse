...
CREATE OR REPLACE FUNCTION search_produits_json(qtext text,magasin integer DEFAULT 1, limit_result int DEFAULT 10)
  RETURNS JSONB
  LANGUAGE plpgsql
AS $$
BEGIN
  RETURN (
    WITH q AS (
      SELECT unaccent(qtext)::text AS query
    )
    ...
    FROM (
           SELECT
             ...
             MAX(
               CASE
                 WHEN q.query ~ '^[0-9]+$' AND (pf.code_cip = q.query OR pf.code_ean = q.query OR p.code_ean_labo = q.query) THEN 1000
                 WHEN q.query ~ '^[0-9]+$' AND (pf.code_cip LIKE q.query || '%' OR pf.code_ean LIKE q.query || '%' OR p.code_ean_labo LIKE q.query || '%') THEN 500
                 ELSE 0
                 END
             ) AS score,
             -- rayons
             ...
             (
               ...
               WHERE sp.produit_id = p.id AND s.magasin_id=magasin
           ...
           WHERE
             ...
             (q.query ~ '^[0-9]+$' AND
              (pf.code_cip = q.query OR pf.code_ean = q.query OR p.code_ean_labo = q.query
                OR pf.code_cip LIKE q.query || '%' OR pf.code_ean LIKE q.query || '%' OR p.code_ean_labo LIKE q.query || '%'))
