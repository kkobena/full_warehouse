...
public class CommandeResource {
    ...
    public ResponseEntity<CommandeLiteDTO> createCommande(
        @Valid @RequestBody CommandeDTO commandeDTO)
        throws URISyntaxException {
        CommandeLiteDTO commande =
            new CommandeLiteDTO(commandService.createNewCommandeFromCommandeDTO(commandeDTO));
        return ResponseEntity.created(new URI("/api/commandes/" + commande.getId()))
            .headers(
                HeaderUtil.createEntityCreationAlert(
                    applicationName, true, ENTITY_NAME, commande.getId().toString()))
            .body(commande);
    }

    @PostMapping("/commandes/add-order-line")
    public ResponseEntity<CommandeLiteDTO> addOrderLine(
        @Valid @RequestBody OrderLineDTO orderLineDTO)
        throws URISyntaxException {
        CommandeLiteDTO result =
            new CommandeLiteDTO(commandService.createOrUpdateOrderLine(orderLineDTO));
    ...
    public ResponseEntity<CommandeLiteDTO> updateQuantityRequested(
        @Valid @RequestBody OrderLineDTO orderLineDTO) throws URISyntaxException {
        CommandeLiteDTO result =
            new CommandeLiteDTO(commandService.updateQuantityRequested(orderLineDTO));
        return ResponseEntity.created(new URI("/api/commandes/" + result.getId()))
            .headers(
                HeaderUtil.createEntityCreationAlert(
                    applicationName, true, ENTITY_NAME, result.getId().toString()))
    ...
    public ResponseEntity<CommandeLiteDTO> updateOrderCostAmount(
        @Valid @RequestBody OrderLineDTO orderLineDTO) throws URISyntaxException {
        CommandeLiteDTO result =
            new CommandeLiteDTO(commandService.updateOrderCostAmount(orderLineDTO));
        return ResponseEntity.created(new URI("/api/commandes/" + result.getId()))
            .headers(
                HeaderUtil.createEntityCreationAlert(
                    applicationName, true, ENTITY_NAME, result.getId().toString()))
    ...
    public ResponseEntity<CommandeLiteDTO> updateOrderUnitPrice(
        @Valid @RequestBody OrderLineDTO orderLineDTO) throws URISyntaxException {
        CommandeLiteDTO result = new CommandeLiteDTO(
            commandService.updateOrderUnitPrice(orderLineDTO));
        return ResponseEntity.created(new URI("/api/commandes/" + result.getId()))
            .headers(
                HeaderUtil.createEntityCreationAlert(
                    applicationName, true, ENTITY_NAME, result.getId().toString()))
    ...
    public ResponseEntity<Void> deleteOrderLinesByIds(
        @PathVariable Long id, @RequestBody List<Long> ids) {
    ...
    public ResponseEntity<VerificationResponseCommandeDTO> importerReponseCommande(
        @PathVariable Long id, @RequestPart("commande") MultipartFile file) {
        VerificationResponseCommandeDTO verificationResponseCommandeDTO =
            commandService.importerReponseCommande(id, file);
