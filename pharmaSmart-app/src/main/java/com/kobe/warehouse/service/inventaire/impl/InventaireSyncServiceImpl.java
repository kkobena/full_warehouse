...
public class InventaireSyncServiceImpl implements InventaireSyncService {
    ...
    public InventaireSyncServiceImpl(
    ...
    public BatchSyncResultRecord synchronize(List<StoreInventoryLineDTO> dtos) {
        ...
        // Résolu une seule fois : toutes les lignes du batch sont comptées par le même utilisateur
        ...
        for (StoreInventoryLineDTO dto : dtos) {
            ...
            // Verrou optimiste : ligne recomptée entre-temps par un autre opérateur.
            // Isolé du reste du batch — les autres lignes sont enregistrées.
            if (isStale(dto, line)) {
                log.warn("Conflit de comptage sur la ligne id={} (version client={}, serveur={})",
                    dto.getId(), dto.getVersion(), line.getVersion());
                conflictedIds.add(dto.getId());
                continue;
            }
            try {
                applyDto(dto, line, countedBy);
                toSave.add(line);
            } catch (Exception e) {
        ...
        return new BatchSyncResultRecord(
            toSave.size(),
            failedIds.size() + conflictedIds.size(),
            failedIds,
            conflictedIds
        );
    }
    ...
    /**
     * Vrai si le client a calculé sa saisie sur une version périmée de la ligne.
     * Version nulle côté client = pas de contrôle (appelants legacy).
     */
    private boolean isStale(StoreInventoryLineDTO dto, StoreInventoryLine line) {
        return dto.getVersion() != null
            && line.getVersion() != null
            && !dto.getVersion().equals(line.getVersion());
    }
