...
$d = @{ host="localhost"; port="5432"; dbName="pharma_smart"; user="pharma_smart"; password=""; schema="pharma_smart" }
...
$form.Text            = "PharmaSmart — Configuration base de données"
...
$note.Text = "Schéma et mot de passe : optionnels (défauts application-prod.yml)."
...
$btnOK.Add_Click({
    $empty = ($txtHost.Text   -match '^\s*$') -or
             ($txtPort.Text   -match '^\s*$') -or
             ($txtDbName.Text -match '^\s*$') -or
             ($txtUser.Text   -match '^\s*$')
    if ($empty) {
        [System.Windows.Forms.MessageBox]::Show(
            "Hôte, port, base de données et utilisateur sont obligatoires.`nSchéma et mot de passe sont optionnels (les defaults de application-prod.yml seront utilisés).",
            "Champs manquants",
