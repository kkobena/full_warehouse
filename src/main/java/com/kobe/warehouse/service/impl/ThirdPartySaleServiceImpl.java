...
public class ThirdPartySaleServiceImpl extends SaleCommonService implements ThirdPartySaleService {
  ...
  public ThirdPartySales computeCarnetAmounts(
    ...
    thirdPartySales.setAmountToBePaid(this.roundedAmount(thirdPartySales.getPartAssure()));
  ...
  public ThirdPartySaleDTO createSale(ThirdPartySaleDTO dto) throws GenericError {
    SalesLine saleLine =
        this.salesLineService.createSaleLineFromDTO(
    ...
    ThirdPartySales sale = this.thirdPartySaleRepository.saveAndFlush(thirdPartySales);
  ...
  private Consommation buildConsommation(Integer montant) {
    ...
    consommation.setId(this.buildConsommationId());
  ...
  public void updateTiersPayantAccount(ThirdPartySaleLine thirdPartySaleLine) {
    ...
    tiersPayant.setUpdatedBy(this.storageService.getUser());
  ...
  private ClientTiersPayant getOneId(Long id) {
    return clientTiersPayantRepository.getOne(id);
  }
  ...
  public SaleLineDTO createOrUpdateSaleLine(SaleLineDTO dto) {
    Optional<SalesLine> salesLineOp =
        this.salesLineService.findBySalesIdAndProduitId(dto.getSaleId(), dto.getProduitId());
    ...
    if (salesLineOp.isPresent()) {
      ...
      this.salesLineService.updateSaleLine(dto, salesLine, storageId);
    ...
    ThirdPartySales thirdPartySales = thirdPartySaleRepository.getOne(dto.getSaleId());
    SalesLine salesLine = this.salesLineService.create(dto, storageId, thirdPartySales);
  ...
  public void deleteSaleLineById(Long id) {
    SalesLine salesLine = this.salesLineService.getOneById(id);
    ...
    this.thirdPartySaleRepository.save(sales);
    this.salesLineService.deleteSaleLine(salesLine);
  }

  public SaleLineDTO updateItemQuantityRequested(SaleLineDTO saleLineDTO)
      throws StockException, DeconditionnementStockOut {
    SalesLine salesLine = this.salesLineService.getOneById(saleLineDTO.getId());
    SalesLine OldSalesLine = (SalesLine) salesLine.clone();
    this.salesLineService.updateItemQuantityRequested(
  ...
  public SaleLineDTO updateItemRegularPrice(SaleLineDTO saleLineDTO) {
    SalesLine salesLine = this.salesLineService.getOneById(saleLineDTO.getId());
    SalesLine OldSalesLine = (SalesLine) salesLine.clone();
    this.salesLineService.updateItemRegularPrice(
        saleLineDTO, salesLine, storageService.getDefaultConnectedUserPointOfSaleStorage().getId());
    Sales sales = salesLine.getSales();
    ThirdPartySales thirdPartySales = (ThirdPartySales) sales;
    computeThirdPartySaleAmounts(thirdPartySales, salesLine, OldSalesLine);
    ...
    this.thirdPartySaleRepository
        ...
        .ifPresent(
            sales -> {
              ...
              this.thirdPartySaleRepository.save(sales);
              this.thirdPartySaleRepository.save(copy);
              List<Ticket> tickets = this.ticketService.findAllBySaleId(sales.getId());
              this.paymentService
                  .findAllBySalesId(sales.getId())
                  .forEach(
                      payment -> {
                        this.paymentService.clonePayment(payment, tickets, copy);
                      });
              this.salesLineService.cloneSalesLine(
                  sales.getSalesLines(),
                  copy,
                  user,
                  storageService.getDefaultConnectedUserPointOfSaleStorage().getId());
              this.findAllBySaleId(id)
                  .forEach(
                      thirdPartySaleLine -> {
                        ThirdPartySaleLine thirdPartySaleLineClone =
                            this.clone(thirdPartySaleLine, copy);
                        this.updateClientTiersPayantAccount(thirdPartySaleLineClone);
                        this.updateTiersPayantAccount(thirdPartySaleLineClone);
                      });
            });
  }
  ...
  public ResponseDTO save(ThirdPartySaleDTO dto)
    ...
    p.getSalesLines()
        .forEach(salesLine -> this.salesLineService.createInventory(salesLine, user, dateD, id));
    ...
    buildReference(p);
    Ticket ticket = this.ticketService.buildTicket(p, dto, user, buildTvaData(p.getSalesLines()));
    this.paymentService.buildPaymentFromFromPaymentDTO(p, dto, ticket, user);
    p.setTvaEmbeded(ticket.getTva());
    List<ThirdPartySaleLine> thirdPartySaleLines = this.findAllBySaleId(p.getId());
    ...
    thirdPartySaleLines.forEach(
        thirdPartySaleLine -> {
          ...
          this.updateClientTiersPayantAccount(thirdPartySaleLine);
          this.updateTiersPayantAccount(thirdPartySaleLine);
        });
  ...
  public ResponseDTO putThirdPartySaleOnHold(ThirdPartySaleDTO dto) {
    ResponseDTO response = new ResponseDTO();
    ThirdPartySales thirdPartySales = thirdPartySaleRepository.getOne(dto.getId());
    paymentService.buildPaymentFromFromPaymentDTO(
        thirdPartySales, dto, this.storageService.getUser());
    thirdPartySaleRepository.save(thirdPartySales);
    response.setSuccess(true);
    return response;
  }
  ...
  public SaleLineDTO updateItemQuantitySold(SaleLineDTO saleLineDTO) {
    SalesLine salesLine = this.salesLineService.getOneById(saleLineDTO.getId());
    this.salesLineService.updateItemQuantitySold(
    ...
    this.thirdPartySaleRepository
        ...
        .ifPresent(
            sales -> {
              ...
              this.thirdPartySaleRepository.delete(sales);
            });
  }
  ...
  public ThirdPartySaleDTO addThirdPartySaleLineToSales(ClientTiersPayantDTO dto, Long saleId)
      throws GenericError, NumBonAlreadyUseException {
    ClientTiersPayant clientTiersPayant = this.clientTiersPayantRepository.getOne(dto.getId());
    ThirdPartySales thirdPartySales = thirdPartySaleRepository.getOne(saleId);
    ...
    return new ThirdPartySaleDTO(this.thirdPartySaleRepository.saveAndFlush(thirdPartySales));
  }

  @Override
  public void removeThirdPartySaleLineToSales(Long clientTiersPayantId, Long saleId) {
    this.thirdPartySaleLineRepository
  ...
  private void computeThirdPartySaleAmounts(
    ...
    this.processDiscount(thirdPartySales, salesLine, OldSalesLine);
    this.computeAllAmounts(thirdPartySales);
  ...
  private ThirdPartySales buildThirdPartySale(ThirdPartySaleDTO dto) throws GenericError {
    ...
    AssuredCustomer assuredCustomer =
        dto.getCustomer() != null
            ? assuredCustomerRepository.getOne(dto.getCustomer().getId())
            : null;
