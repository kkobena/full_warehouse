    ...
    SELECT
        ...
        CASE
            WHEN c.receipt_date IS NOT NULL AND c.order_date IS NOT NULL THEN
                EXTRACT(DAY FROM (c.receipt_date - c.order_date))
            ELSE NULL
        END as delivery_days,
        -- Conformity: sum of received vs requested
...
delivery_metrics AS (
    SELECT
        ...
        -- Conformity rate: (total received / total requested) * 100
        CASE
...
SELECT
    ...
    -- Performance score (weighted average: 40% volume, 30% delivery speed, 30% conformity)
    CASE
        WHEN r12.purchase_amount_12m > 0 THEN
            ROUND(
                (LEAST(r12.purchase_amount_12m / 10000000.0, 1) * 40) + -- Volume score (max 40 points)
                (GREATEST(1 - (COALESCE(dm.avg_delivery_days, 30) / 30.0), 0) * 30) + -- Delivery speed (max 30 points)
                (COALESCE(dm.conformity_rate_pct, 0) * 0.3), -- Conformity (max 30 points)
                2
            )
        ELSE 0
