...
public class ExportationCsvService {
      ...
      commande
          ...
          .forEach(
              orderLine -> {
                ...
                try {
                  printer.printRecord(
                      StringUtils.isNotEmpty(produit.getCodeEan())
                          ? produit.getCodeEan()
  ...
  public void createRuptureFile(
      String reference, List<OrderItem> items, CommandeModel commandeModel) {
    log.info("createRuptureFile data to the csv printer");
    String filename =
        this.fileStorageLocation
            .resolve("rupture_" + reference+ LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy_MM_dd")) + ".csv")
            .toFile()
            .getAbsolutePath();
    try (final FileWriter writer = new FileWriter(filename);
  ...
  private void printCOPHARMEDFormatCsv(CSVPrinter printer, List<OrderItem> items)
      throws IOException {
    printer.printRecord(
        "Date",
        "Numero Facture",
