...
public class CommandeDataServiceImpl implements CommandeDataService {
    ...
    private final Comparator<OrderLineDTO> comparingByProduitLibelle = Comparator
        .comparing(OrderLineDTO::getProduitLibelle);
    private final Comparator<OrderLineDTO> comparingByProduitCip = Comparator
        .comparing(OrderLineDTO::getProduitCip);
    private final Comparator<OrderLineDTO> comparingByDateUpdated = Comparator
        .comparing(OrderLineDTO::getUpdatedAt, Comparator.reverseOrder());
    ...
    public List<OrderLineDTO> filterCommandeLines(CommandeFilterDTO commandeFilter) {
        ...
        if (StringUtils.isNotEmpty(commandeFilter.getSearch())) {
            if (commandeFilter.getFilterCommaneEnCours() != null
                ...
                != FilterCommaneEnCours.ALL) {
                switch (commandeFilter.getFilterCommaneEnCours()) {
                    case NOT_EQUAL:
                        return orderLines.stream()
                            .filter(
                                orderLine ->
                                    searchFilterPrix.test(
                                        orderLine, commandeFilter.getFilterCommaneEnCours()))
                            .filter(
                                orderLine -> searchPredicate.test(orderLine, commandeFilter.getSearch()))
    ...
    private Comparator<OrderLineDTO> getSort(Sort sort) {
        ...
        return switch (sort) {
            ...
            case UPDATE, null -> comparingByDateUpdated;
        };
    }
}
