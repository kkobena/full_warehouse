...
public interface StockProduitRepository extends JpaRepository<StockProduit, Long> {
    ...
    @Query(value = "SELECT SUM(o.qty_ug+o.qty_stock) AS totalStock,o.produit_id AS produitId FROM stock_produit o WHERE o.produit_id NOT IN (SELECT p.produit_id FROM daily_stock p WHERE p.date_key =:now) GROUP BY o.produit_id LIMIT :limit",nativeQuery = true)
