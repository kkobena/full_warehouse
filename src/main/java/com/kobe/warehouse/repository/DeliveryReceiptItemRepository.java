...
public interface DeliveryReceiptItemRepository extends JpaRepository<DeliveryReceiptItem, Long> {
    ...
    @Query(
        value = "SELECT SUM(o.quantity_received) AS quantite,SUM(o.cost_amount*o.quantity_received) AS montantAchat  FROM delivery_receipt_item o JOIN fournisseur_produit fp ON o.fournisseur_produit_id = fp.id JOIN delivery_receipt d ON o.delivery_receipt_id = d.id WHERE fp.produit_id =:produitId AND d.receipt_status=:statut AND DATE(o.updated_date) BETWEEN :startDate AND :endDate",
        nativeQuery = true
    )
