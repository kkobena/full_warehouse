...
public class ThirdPartySaleResource {
  ...
  public ResponseEntity<ThirdPartySaleDTO> createSale(
      @Valid @RequestBody ThirdPartySaleDTO thirdPartySaleDTO, HttpServletRequest request)
      throws URISyntaxException {
  ...
  public ResponseEntity<ResponseDTO> closeCashSale(
      @Valid @RequestBody ThirdPartySaleDTO thirdPartySaleDTO) throws URISyntaxException {
    if (thirdPartySaleDTO.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
    }
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, thirdPartySaleDTO.getId().toString()))
        .body(result);
  }
  ...
  public ResponseEntity<SaleLineDTO> updateItemQtyRequested(
    ...
    return ResponseEntity.created(
            new URI("/api/sales/update-item/quantity-requested/assurance" + result.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, result.getId().toString()))
        .body(result);
  ...
  public ResponseEntity<SaleLineDTO> updateItemPrice(@Valid @RequestBody SaleLineDTO saleLineDTO)
    ...
    return ResponseEntity.created(
            new URI("/api/sales/update-item/price/assurance" + result.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, result.getId().toString()))
        .body(result);
  ...
  public ResponseEntity<SaleLineDTO> updateItemQtySold(@Valid @RequestBody SaleLineDTO saleLineDTO)
    ...
    return ResponseEntity.created(
            new URI("/api/sales/update-item/quantity-sold/assurance" + result.getId()))
        .headers(
            HeaderUtil.createEntityCreationAlert(
                applicationName, true, ENTITY_NAME, result.getId().toString()))
        .body(result);
  ...
  public ResponseEntity<Void> removeThirdPartySaleLineToSales(
      @PathVariable("id") Long clientTiersPayantId, @PathVariable("saleId") Long saleId) {
    saleService.removeThirdPartySaleLineToSales(clientTiersPayantId, saleId);
    return ResponseEntity.noContent()
        .headers(
            HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, saleId.toString()))
        .build();
  }

  @PutMapping("/sales/add-assurance/assurance/{id}")
  public ResponseEntity<ThirdPartySaleDTO> addThirdPartySaleLineToSales(
      @PathVariable Long id, @Valid @RequestBody ClientTiersPayantDTO dto)
      throws URISyntaxException {
