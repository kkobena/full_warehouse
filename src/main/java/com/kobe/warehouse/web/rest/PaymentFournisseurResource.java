...
public class PaymentFournisseurResource {
   ...
   *     paymentFournisseur, or with status {@code 400 (Bad Request)} if the paymentFournisseur has
   *     already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/payment-fournisseurs")
  public ResponseEntity<PaymentFournisseur> createPaymentFournisseur(
      @Valid @RequestBody PaymentFournisseur paymentFournisseur) throws URISyntaxException {
    log.debug("REST request to save PaymentFournisseur : {}", paymentFournisseur);
    if (paymentFournisseur.getId() != null) {
      throw new BadRequestAlertException(
          "A new paymentFournisseur cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     paymentFournisseur, or with status {@code 400 (Bad Request)} if the paymentFournisseur is
   *     not valid, or with status {@code 500 (Internal Server Error)} if the paymentFournisseur
   *     couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/payment-fournisseurs")
  public ResponseEntity<PaymentFournisseur> updatePaymentFournisseur(
      @Valid @RequestBody PaymentFournisseur paymentFournisseur) throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, paymentFournisseur.getId().toString()))
        .body(result);
  }
   ...
   *     paymentFournisseur, or with status {@code 404 (Not Found)}.
   */
