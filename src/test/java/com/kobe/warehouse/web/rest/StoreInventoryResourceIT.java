...
public class StoreInventoryResourceIT {
  ...
  @Autowired private StoreInventoryRepository storeInventoryRepository;

  @Autowired private EntityManager em;

  @Autowired private MockMvc restStoreInventoryMockMvc;
  ...
  public void createStoreInventory() throws Exception {
    ...
    restStoreInventoryMockMvc
        .perform(
            post("/api/store-inventories")
                .with(csrf())
    ...
    assertThat(testStoreInventory.getInventoryValueCostBegin())
        .isEqualTo(DEFAULT_INVENTORY_VALUE_COST_BEGIN);
    assertThat(testStoreInventory.getInventoryAmountBegin())
        .isEqualTo(DEFAULT_INVENTORY_AMOUNT_BEGIN);
    ...
    assertThat(testStoreInventory.getInventoryValueCostAfter())
        .isEqualTo(DEFAULT_INVENTORY_VALUE_COST_AFTER);
    assertThat(testStoreInventory.getInventoryAmountAfter())
        .isEqualTo(DEFAULT_INVENTORY_AMOUNT_AFTER);
  }
  ...
  public void getAllStoreInventories() throws Exception {
    ...
    restStoreInventoryMockMvc
        .perform(get("/api/store-inventories?sort=id,desc"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(jsonPath("$.[*].id").value(hasItem(storeInventory.getId().intValue())))
        .andExpect(
            jsonPath("$.[*].inventoryValueCostBegin")
                .value(hasItem(DEFAULT_INVENTORY_VALUE_COST_BEGIN.intValue())))
        .andExpect(
            jsonPath("$.[*].inventoryAmountBegin")
                .value(hasItem(DEFAULT_INVENTORY_AMOUNT_BEGIN.intValue())))
        .andExpect(jsonPath("$.[*].createdAt").value(hasItem(DEFAULT_CREATED_AT.toString())))
        .andExpect(jsonPath("$.[*].updatedAt").value(hasItem(DEFAULT_UPDATED_AT.toString())))
        .andExpect(
            jsonPath("$.[*].inventoryValueCostAfter")
                .value(hasItem(DEFAULT_INVENTORY_VALUE_COST_AFTER.intValue())))
        .andExpect(
            jsonPath("$.[*].inventoryAmountAfter")
                .value(hasItem(DEFAULT_INVENTORY_AMOUNT_AFTER.intValue())));
  }
  ...
  public void getStoreInventory() throws Exception {
    ...
    restStoreInventoryMockMvc
        .perform(get("/api/store-inventories/{id}", storeInventory.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(jsonPath("$.id").value(storeInventory.getId().intValue()))
        .andExpect(
            jsonPath("$.inventoryValueCostBegin")
                .value(DEFAULT_INVENTORY_VALUE_COST_BEGIN.intValue()))
        .andExpect(
            jsonPath("$.inventoryAmountBegin").value(DEFAULT_INVENTORY_AMOUNT_BEGIN.intValue()))
        .andExpect(jsonPath("$.createdAt").value(DEFAULT_CREATED_AT.toString()))
        .andExpect(jsonPath("$.updatedAt").value(DEFAULT_UPDATED_AT.toString()))
        .andExpect(
            jsonPath("$.inventoryValueCostAfter")
                .value(DEFAULT_INVENTORY_VALUE_COST_AFTER.intValue()))
        .andExpect(
            jsonPath("$.inventoryAmountAfter").value(DEFAULT_INVENTORY_AMOUNT_AFTER.intValue()));
  }
  ...
  public void getNonExistingStoreInventory() throws Exception {
    // Get the storeInventory
    restStoreInventoryMockMvc
        .perform(get("/api/store-inventories/{id}", Long.MAX_VALUE))
  ...
  public void updateStoreInventory() throws Exception {
    ...
    StoreInventory updatedStoreInventory =
        storeInventoryRepository.findById(storeInventory.getId()).get();
    // Disconnect from session so that the updates on updatedStoreInventory are not directly saved
    // in db
    ...
    StoreInventory testStoreInventory = storeInventoryList.get(storeInventoryList.size() - 1);
    assertThat(testStoreInventory.getInventoryValueCostBegin())
        .isEqualTo(UPDATED_INVENTORY_VALUE_COST_BEGIN);
    assertThat(testStoreInventory.getInventoryAmountBegin())
        .isEqualTo(UPDATED_INVENTORY_AMOUNT_BEGIN);
    ...
    assertThat(testStoreInventory.getInventoryValueCostAfter())
        .isEqualTo(UPDATED_INVENTORY_VALUE_COST_AFTER);
    assertThat(testStoreInventory.getInventoryAmountAfter())
        .isEqualTo(UPDATED_INVENTORY_AMOUNT_AFTER);
  }
  ...
  public void deleteStoreInventory() throws Exception {
    ...
    restStoreInventoryMockMvc
        .perform(
            delete("/api/store-inventories/{id}", storeInventory.getId())
                .with(csrf())
