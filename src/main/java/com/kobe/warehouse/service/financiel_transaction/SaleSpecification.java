...
public class SaleSpecification {
    ...
    public Specification<Sales> builder(MvtParam mvtParam) {
        return (root, _, cb) -> {
            ...
            predicates.add(cb.between(cb.function("DATE", java.time.LocalDate.class, root.get(Sales_.updatedAt)), mvtParam.getFromDate(), mvtParam.getToDate()));
            ...
            predicates.add(root.get(Sales_.type).in(mvtParam.getTypeVentes().stream().map(com.kobe.warehouse.service.cash_register.dto.TypeVente::getValue).toList()));
